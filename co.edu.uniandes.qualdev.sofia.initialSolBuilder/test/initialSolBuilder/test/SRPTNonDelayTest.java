...
public class SRPTNonDelayTest {
	...
	public void setupScenario() throws Exception{
		...
		solution = testRule.createInitialSolution(problemFiles, betas , "structure.factory.impl.VectorFactory", null, new SequencialDecoding());
	...
	@Test
	public void testsRPTNonDelay04x04x02_1() throws Exception{
		ArrayList<String> problemFiles = new ArrayList<String>();
		...
		Assert.assertEquals("La primer posicion deberia ser las <0,3,6>",vector.getOperations().get(0), new Operation(new OperationIndexVO(0, 0, 3, 6)));
		Assert.assertEquals("La primer posicion deberia comenzar en 2 y terminar en 14",vector.getOperations().get(0).getInitialTime()==2 && vector.getOperations().get(0).getFinalTime()==14, true );
		...
		Assert.assertEquals("La cuarta posicion deberia ser las <1,1,3>",vector.getOperations().get(3), new Operation(new OperationIndexVO(0, 1, 1, 3)));
		Assert.assertEquals("La cuarta posicion deberia comenzar en 5 y terminar en 19",vector.getOperations().get(3).getInitialTime()==5 && vector.getOperations().get(3).getFinalTime()==19, true );
		...
		Assert.assertEquals("La decima posicion deberia comenzar en 32 y terminar en 42",vector.getOperations().get(9).getInitialTime()==31 && vector.getOperations().get(9).getFinalTime()==41, true );
